...
local type, next, pairs, ipairs, loadfile, table = type, next, pairs, ipairs, loadfile, table
...
function protocol.del_interface(self, ifname)
    ...
    if ifname and not self:is_floating() then
        ...
        local wif = _wifi_lookup(ifname)
        if wif then
            _filter("wireless", wif, "network", self.sid)
        end
