...
local function requestProxy()
    ...
    if settings then
        ...
        settings[key]["user"] = user or "''"
        settings[key]["passwd"] = password or "''"
        settings[key]["server"] = server or "''"
        settings[key]["node"] = node or "''"
        settings[key]["localport"] = "6543" .. createIndex
    else
        luci.http.prepare_content("application/json")
...
local function getDeviceList()
    ...
    --返回值
    luci.http.prepare_content("application/json")
    luci.http.write_json({ code = 200, deviceList = deviceList })
end
